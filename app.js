...
const SIM_ERS_DRAIN_AT_99_PER_SEC = 0.5;
...
const SIM_ERS_TAPER_TO_BRAKE_DISTANCE = 24;
const SIM_ERS_BOOST_RAMP_UP_KMH_PER_SEC = 42;
const SIM_ERS_BOOST_RAMP_DOWN_KMH_PER_SEC = 68;
const SIM_ERS_MIN_READY_BATTERY_PERCENT = 40;
const SIM_ERS_TOP_SPEED_MARGIN_KMH = 1.5;
const SIM_ERS_GAP_LT1_CHARGE_MIN = 20;
const SIM_ERS_GAP_LT1_CHARGE_MAX = 30;
const SIM_ERS_GAP_1_TO_2_CHARGE_MIN = 40;
const SIM_ERS_GAP_1_TO_2_CHARGE_MAX = 60;
const SIM_ERS_GAP_2_TO_3_CHARGE_MIN = 80;
const SIM_ERS_GAP_2_TO_3_CHARGE_MAX = 90;
const SIM_ERS_DEPLOY_GAP_DISTANCE_PATH = 260;
const SIM_ERS_FULL_BOOST_DISTANCE_PATH = 70;
...
function renderSimRacingPreview() {
  ...
  if (!simState.running) {
    status.textContent = `Monza preview is ready. ERS auto deploy uses distance to car ahead (<= ${SIM_ERS_DEPLOY_GAP_DISTANCE_PATH.toFixed(0)} path units).`;
  }
...
function startSimRacingAnimation() {
  ...
  simState.teamRuns = buildSimTeamRuns(simState.teamsOnGrid).map(run => {
    ...
    return {
      ...
      ersActive: false,
      ersBoostKmh: 0,
      currentErsBoostKmh: 0,
      ersChargeTargetLap: 0,
      ersChargeTargetPercent: SIM_ERS_MIN_READY_BATTERY_PERCENT,
      ersRechargeLocked: false,
      ersSpeedGateReady: false,
      drsActive: false,
      drsZoneId: null,
      drsEndDistance: null,
  ...
  setSimStatus('Simulation preview running. ERS deploy is distance-based to the car ahead, fades smoothly, and is locked OFF in Turn 1/Turn 2 braking zones.');
...
function renderSimDriverTiming(runs, elapsedSec, lapLength) {
  ...
  legend.innerHTML = ordered.map((entry, idx) => {
    ...
    const ersArmTarget = Number(entry.run.ersChargeTargetPercent ?? SIM_ERS_MIN_READY_BATTERY_PERCENT);
    ...
    const ersLabel = entry.run.ersActive
      ...
      : 'ERS OFF';
    const speedGateLabel = entry.run.ersSpeedGateReady ? 'VMAX OK' : 'VMAX WAIT';

    return `
      <span class="sim-legend-item">
        <span class="sim-legend-dot" style="background:${entry.run.team.color}"></span>
        <span>P${idx + 1} · ${escHtml(entry.run.tag)} · ${escHtml(getSimRunDisplayName(entry.run))} · ${timing} · ${shownSpeed} km/h · BAT ${ersBattery.toFixed(1)}% (ARM ${ersArmTarget.toFixed(1)}%) · ${speedGateLabel} · ${drsLabel} · ${ersLabel} · L${lap} · ${lapTime.toFixed(2)}s/lap</span>
      </span>
    `;
  }).join('');
}
...
function getErsChargeTargetRangeByGap(gapAheadSec) {
  if (Number.isFinite(gapAheadSec) && gapAheadSec < 1) {
    return [SIM_ERS_GAP_LT1_CHARGE_MIN, SIM_ERS_GAP_LT1_CHARGE_MAX];
  }

  if (Number.isFinite(gapAheadSec) && gapAheadSec < 2) {
    return [SIM_ERS_GAP_1_TO_2_CHARGE_MIN, SIM_ERS_GAP_1_TO_2_CHARGE_MAX];
  }

  if (Number.isFinite(gapAheadSec) && gapAheadSec < 3) {
    return [SIM_ERS_GAP_2_TO_3_CHARGE_MIN, SIM_ERS_GAP_2_TO_3_CHARGE_MAX];
  }

  return [SIM_ERS_MIN_READY_BATTERY_PERCENT, SIM_ERS_GAP_1_TO_2_CHARGE_MAX];
}

function buildRunUniqueBias(run) {
  const base = Number(run.ersRandomSeed) || 1;
  return ((base * 73) % 997) / 997;
}
...
function pseudoRandom01(seedNumber) {
  const x = Math.sin(seedNumber * 12.9898 + 78.233) * 43758.5453;
  return x - Math.floor(x);
}

function pickLapChargeTargetPercent(run, lapNumber, gapAheadSec) {
  const [minTarget, maxTarget] = getErsChargeTargetRangeByGap(gapAheadSec);
  const span = Math.max(0, maxTarget - minTarget);
  const usedTargets = new Set(
    simState.teamRuns
      .filter(other => other !== run && Number(other.ersChargeTargetLap) === lapNumber)
      .map(other => Number(other.ersChargeTargetPercent).toFixed(2))
  );

  const baseSeed = Number(run.ersRandomSeed) || 1;
  const bias = buildRunUniqueBias(run);
  const lapSeed = baseSeed + (Number(lapNumber) || 1) * 131;
  let candidate = minTarget + span * ((pseudoRandom01(lapSeed) * 0.82) + (bias * 0.18));
  let attempts = 0;

  while (usedTargets.has(candidate.toFixed(2)) && attempts < 16) {
    candidate = minTarget + span * pseudoRandom01(lapSeed + (attempts + 1) * 37);
    attempts += 1;
  }

  if (usedTargets.has(candidate.toFixed(2))) {
    candidate = minTarget + span * bias;
  }

  return Number(candidate.toFixed(2));
}

function getErsBrakeLimiter(distanceOnLap, lapLength) {
  const turnDistances = [simState.turn1Distance, simState.turn2Distance];
  let limiter = 1;

  for (const turnDistance of turnDistances) {
    if (turnDistance == null) continue;
    const forwardToTurn = getForwardDistanceOnLap(distanceOnLap, turnDistance, lapLength);

    // Hard lock ERS inside braking phase for the chicane.
    if (forwardToTurn <= SIM_BRAKE_PREP_DISTANCE) {
      return 0;
    }

    // Fade ERS down before entering the braking phase.
    const taperStart = SIM_BRAKE_PREP_DISTANCE + SIM_ERS_TAPER_TO_BRAKE_DISTANCE;
    if (forwardToTurn <= taperStart) {
      const ratio = (forwardToTurn - SIM_BRAKE_PREP_DISTANCE) / SIM_ERS_TAPER_TO_BRAKE_DISTANCE;
      limiter = Math.min(limiter, Math.max(0, Math.min(1, ratio)));
    }
  }
...
function getErsDistanceFactor(gapAheadDistance) {
  if (!Number.isFinite(gapAheadDistance) || gapAheadDistance <= 0) return 0;
  if (gapAheadDistance <= SIM_ERS_FULL_BOOST_DISTANCE_PATH) return 1;
  if (gapAheadDistance >= SIM_ERS_DEPLOY_GAP_DISTANCE_PATH) return 0;

  const span = SIM_ERS_DEPLOY_GAP_DISTANCE_PATH - SIM_ERS_FULL_BOOST_DISTANCE_PATH;
  const ratio = (SIM_ERS_DEPLOY_GAP_DISTANCE_PATH - gapAheadDistance) / Math.max(span, 0.001);
...
function advanceSimRuns(dtSec, lapLength) {
  ...
  const gapAheadDistanceByRun = new Map();
  orderedRuns.forEach((run, idx) => {
    if (idx === 0) {
      gapAheadSecByRun.set(run, Number.POSITIVE_INFINITY);
      gapAheadDistanceByRun.set(run, Number.POSITIVE_INFINITY);
      return;
    }
    ...
    gapAheadDistanceByRun.set(run, gapDistance);
  ...
  simState.teamRuns.forEach(run => {
    ...
    const currentLap = Math.max(1, Math.floor(run.currentDistance / lapLength) + 1);
    ...
    const gapAheadDistance = gapAheadDistanceByRun.get(run);
    ...
    if (Number(run.ersChargeTargetLap) !== currentLap) {
      run.ersChargeTargetLap = currentLap;
      run.ersChargeTargetPercent = pickLapChargeTargetPercent(run, currentLap, gapAheadSec);
    }

    const requiredReadyBattery = Number(run.ersChargeTargetPercent) || SIM_ERS_MIN_READY_BATTERY_PERCENT;
    const topSpeedReady = currentSpeed >= (run.speedKmh - SIM_ERS_TOP_SPEED_MARGIN_KMH);

    if (currentBattery <= SIM_ERS_MIN_BATTERY_TO_DEPLOY) {
      run.ersRechargeLocked = true;
    } else if (run.ersRechargeLocked && currentBattery >= requiredReadyBattery) {
      run.ersRechargeLocked = false;
    }

    const batteryReadyForDeploy = !run.ersRechargeLocked && currentBattery > SIM_ERS_MIN_BATTERY_TO_DEPLOY;
    const withinDistanceWindow =
      Number.isFinite(gapAheadDistance) &&
      gapAheadDistance > 0 &&
      gapAheadDistance <= SIM_ERS_DEPLOY_GAP_DISTANCE_PATH;
    const ersDistanceFactor = getErsDistanceFactor(gapAheadDistance);

    const shouldRequestErs =
      withinDistanceWindow &&
      topSpeedReady &&
      batteryReadyForDeploy;

    const brakeLimiter = getErsBrakeLimiter(wrappedDistance, lapLength);
    const maxErsBoost = getErsBoostKmh(ersDeployRating);
    const targetErsBoost = shouldRequestErs ? maxErsBoost * brakeLimiter * ersDistanceFactor : 0;
    const currentErsBoost = Math.max(0, Number(run.currentErsBoostKmh ?? run.ersBoostKmh) || 0);

    let nextErsBoost = currentErsBoost;
    if (targetErsBoost > currentErsBoost) {
      nextErsBoost = Math.min(targetErsBoost, currentErsBoost + SIM_ERS_BOOST_RAMP_UP_KMH_PER_SEC * dtSec);
    } else if (targetErsBoost < currentErsBoost) {
      nextErsBoost = Math.max(targetErsBoost, currentErsBoost - SIM_ERS_BOOST_RAMP_DOWN_KMH_PER_SEC * dtSec);
    }
    ...
    const ersUsageRatio = maxErsBoost > 0 ? Math.max(0, Math.min(1, nextErsBoost / maxErsBoost)) : 0;
    const ersInUse = nextErsBoost > 0.05;
    const baseSpeedWithErs = drsTopSpeedKmh + nextErsBoost;
    ...
    if (ersInUse) {
      nextBattery -= getErsDrainRatePercentPerSec(ersDeployRating) * ersUsageRatio * dtSec;
    } else {
      nextBattery += getErsChargeRatePercentPerSec(ersDeployRating) * dtSec;
    }
    nextBattery = clampSimBattery(nextBattery);

    if (nextBattery <= SIM_ERS_MIN_BATTERY_TO_DEPLOY) {
      run.ersRechargeLocked = true;
    } else if (run.ersRechargeLocked && nextBattery >= requiredReadyBattery) {
      run.ersRechargeLocked = false;
    }
    ...
    run.ersActive = ersInUse && !run.ersRechargeLocked && nextBattery > SIM_ERS_MIN_BATTERY_TO_DEPLOY;
    run.currentErsBoostKmh = run.ersActive ? nextErsBoost : 0;
    run.ersBoostKmh = run.currentErsBoostKmh;
    run.ersSpeedGateReady = topSpeedReady;
    run.drsTopSpeedKmh = drsTopSpeedKmh;
    run.gapAheadSec = Number.isFinite(gapAheadSec) ? gapAheadSec : null;
    run.gapAheadDistance = Number.isFinite(gapAheadDistance) ? gapAheadDistance : null;
  });
}
...
function buildSimTeamRuns(teamsOnGrid) {
  ...
  return runs.map((runObj, idx) => {
    ...
    const runSeedSource = [
      team.seedId || team.name || team.id,
      runObj.driver?.seedId || runObj.driver?.id || `slot-${idx}`,
      runObj.tag,
      idx
    ].join('|');
    const ersRandomSeed = hashStringToPositiveInt(runSeedSource);
